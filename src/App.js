...
function App() {
  ...
  const updateTime = () => {
    ...
    timeStamp = `${year}-${month}-${date} ` + timeStamp
    return timeStamp
  }
  const currentTime = () => {
    let currentTime = new Date();
...
// JSX starts here
  return (
    ...
    <div className={`App ${temperatureConverter(weather?.list[0].main.temp) <= 50 ? "cold" : ""}`}>
      <main>
        <div className="input">
          <input
            type="text"
        ...
        {!weather?.list[0].main.temp ? <h1>See the upcoming forecast </h1> : <h1>The Date is {timeStamp} UTC</h1>}
        {loading ? (
        <h1>Processing...</h1>
          ...
          <>
            ...
            <div className="weather-box">
              ...
              <div className="weather">It looks like {weather?.list[0].weather[0].main}</div>
              <ul>
